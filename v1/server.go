...
import (
	...
	"github.com/ahlusar1989/scheduling_service/v1/backends/result"
	"github.com/ahlusar1989/scheduling_service/v1/brokers/eager"
	"github.com/ahlusar1989/scheduling_service/v1/config"
	"github.com/ahlusar1989/scheduling_service/v1/tasks"
	"github.com/ahlusar1989/scheduling_service/v1/tracing"
	"github.com/satori/go.uuid"

	backendsiface "github.com/ahlusar1989/scheduling_service/v1/backends/iface"
	brokersiface "github.com/ahlusar1989/scheduling_service/v1/brokers/iface"
	opentracing "github.com/opentracing/opentracing-go"
)

// Server is the main Scheduling Service object and stores all configuration
...
func (server *Server) SendTaskWithContext(ctx context.Context, signature *tasks.Signature) (*result.AsyncResult, error) {
	span, _ := opentracing.StartSpanFromContext(ctx, "SendTask", tracing.ProducerOption(), tracing.SchedulingTag)
	defer span.Finish()
...
func (server *Server) SendChainWithContext(ctx context.Context, chain *tasks.Chain) (*result.ChainAsyncResult, error) {
	span, _ := opentracing.StartSpanFromContext(ctx, "SendChain", tracing.ProducerOption(), tracing.SchedulingTag, tracing.WorkflowChainTag)
	defer span.Finish()
...
func (server *Server) SendGroupWithContext(ctx context.Context, group *tasks.Group, sendConcurrency int) ([]*result.AsyncResult, error) {
	span, _ := opentracing.StartSpanFromContext(ctx, "SendGroup", tracing.ProducerOption(), tracing.SchedulingTag, tracing.WorkflowGroupTag)
	defer span.Finish()
...
func (server *Server) SendChordWithContext(ctx context.Context, chord *tasks.Chord, sendConcurrency int) (*result.ChordAsyncResult, error) {
	span, _ := opentracing.StartSpanFromContext(ctx, "SendChord", tracing.ProducerOption(), tracing.SchedulingTag, tracing.WorkflowChordTag)
	defer span.Finish()
